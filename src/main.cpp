...
void moveMotorX(long zielPos, int speed, int acceleration) {    // acceleration 0-10 // speed 0 - 10
...
void stallDetect(uint32_t ms)  {
  ...
  if((ms-last_time) > 50) { //run every 0.05s
    ...
    if (drv_status.sg_result == 0 && modus == -2) {   //zurückfahen (modus -2)
      ...
      timerAlarmDisable(timer);
      digitalWrite(EN_PIN,HIGH);
      leangeSlider = SliderPosition - 2000;
      modus = 0;
      driver.en_pwm_mode(true);       // Toggle stealthChop on TMC2130/2160/5130/5160
      driver.reset();
      Serial.println(leangeSlider);
      shaft = !shaft;
      driver.shaft(shaft);
...
void start(OSCMessage &msg, int addrOffset ) { // Slider Position modus 1
  ...
  OSCMessage msgOUT("/modus_1/slider_fader");
    msgOUT.add(float(SliderPosition)/float(leangeSlider)); 
    Serial.println(float(SliderPosition/leangeSlider));     //Slider Position Variable
    Udp.beginPacket(Udp.remoteIP(), destPort);    //Sendet den Wert der am Encoder eingestellt wurde an den Anzeiger für die Position
    msgOUT.send(Udp); // send the bytes
    Udp.endPacket(); // mark the end of the OSC Packet
    msgOUT.empty(); // free space occupied by message
...
void checkRecSliderPos(bool check,float pos) {
  ...
  if (check == false) {
    ...
    long mappedZiel = posSaved*float(leangeSlider);
  
    moveMotorX(mappedZiel,speedX,10);
